    ...
    for (int i = 0; i < n; i++) {
        ...
        while (true) {
            ...
            if (prevPrevMove != MOVE_N && prevMove != MOVE_N &&
                face(currMove) == face(prevPrevMove) && isOpposingFace(currMove, prevMove))
                continue;

            // Block same-face repeats
            if (prevMove != MOVE_N && face(currMove) == face(prevMove)) continue;
...
bool Cube::is_solved() const {
    for(int i=0;i<CORNERS_N;++i) if(cp[i]!=solvedCp[i]||co[i]!=solvedCo[i]) return false;
    for(int i=0;i<EDGES_N;  ++i) if(ep[i]!=solvedEp[i]||eo[i]!=solvedEo[i]) return false;
    return true;
}
