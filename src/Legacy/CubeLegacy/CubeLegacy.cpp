...
void Cube::switch_pieces(PieceType pieceType, PiecePart piecePart, uint64_t& state, const std::array<unsigned char, 4>& idxList, unsigned char rot) const {
    size_t i;
...
std::vector<Move> Cube::genScramble(int n) const {
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (true) {
            ...
            if (face(currMove) == face(prevPrevMove) && isOpposingFace(currMove, prevMove))
                continue;

            // Block same-face repeats
            if (face(currMove) == face(prevMove))
                continue;
...
Cube& Cube::doMove(Move m) { singleMove(m, cornerState, edgeState); return *this; }
Cube& Cube::undoMove(Move m) { singleMove(inverse(m), cornerState, edgeState); return *this; }
