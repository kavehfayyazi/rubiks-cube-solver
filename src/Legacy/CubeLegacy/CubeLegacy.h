...
private:
    ...
    void switch_pieces(PieceType pieceType, PiecePart piecePart, uint64_t& state, const std::array<unsigned char, 4>& idxList, unsigned char rot) const;
...
public:
    ...
    Cube& doMove (Move move);
    Cube& undoMove (Move move);
...
private:
    ...
    static inline constexpr uint64_t solvedCornerState = [] {
        uint64_t s = 0;
...
d
    static inline constexpr uint64_t solvedEdgeState = [] {
        uint64_t s = 0;
