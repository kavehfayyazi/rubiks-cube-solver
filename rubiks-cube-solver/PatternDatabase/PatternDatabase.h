...
private:
    ...
    void generatePDB(PieceType pieceType, std::vector<uint8_t>& pdb, unsigned char maxDepth, const std::optional<std::array<unsigned char, 6>>& edgesArr);
...
private:
    ...
    static constexpr std::array<uint64_t, 8> FACTORIAL = {1,1,2,6,24,120,720,5040,40320}; // 0-8

    static uint64_t NUM_CORNER_PERMUTATIONS = FACTORIAL[8];
    static uint64_t NUM_CORNER_ORIENTATIONS = 2187;
    static uint64_t CORNER_TOTAL = NUM_CORNER_PERMUTATIONS * NUM_CORNER_ORIENTATIONS;

    static uint64_t NUM_EDGE_PERMUTATIONS = FACTORIAL[6];
    static uint64_t NUM_EDGE_ORIENTATIONS = 64;
    static uint64_t EDGE_TOTAL = NUM_EDGE_PERMUTATIONS * NUM_EDGE_ORIENTATIONS;
};
